...
### 阶段一：规划 (Planning)
1. **接收需求**：收到用户初始需求后，第一个行动是检查 plan.md 文件状态
   - **如果 plan.md 已存在且有内容**：读取现有内容，保留项目核心信息和已有任务面板，在此基础上更新或补充任务
   - **如果 plan.md 不存在或为空**：创建新的任务面板
   - **重要**：严禁删除已有内容后重建，必须保留并更新现有内容
...
4. **更新结果**：
   ...
   - **重要**：更新 plan.md 时，必须保留任务面板的完整结构，只更新对应任务行的状态列、输出/产物列和备注列，不要删除或重建整个任务面板
...
**文件更新原则**：
- **保留已有内容**：如果 plan.md 已存在且有内容，必须读取并保留现有内容，在此基础上更新
- **禁止删除重建**：严禁删除整个文件内容后重建，必须采用增量更新的方式
- **更新范围**：
  - 保留：项目核心信息、任务面板结构、已有任务行
  - 更新：任务状态列、输出/产物列、备注/错误信息列
  - 补充：可以添加新任务行，但不要删除已有任务行
...
**任务面板更新操作指引：**
1. **读取现有内容**：更新前必须先读取 plan.md 的完整内容，了解当前任务面板状态
2. **定位任务行**：根据任务 ID 找到需要更新的任务行
3. **更新指定列**：只更新以下列的内容：
   - **状态列**：根据任务执行情况更新状态（⚪️/🟡/🟢/🔴/⏸️/🔵）
   - **输出/产物列**：任务完成后填写产出物
   - **备注/错误信息列**：记录执行过程中的关键信息或错误日志
4. **保留其他内容**：必须保留以下内容不变：
   - 任务 ID、任务名称、依赖关系、输入/参数列
   - 任务面板的表头结构
   - 项目核心信息部分
   - 其他任务行的内容
5. **禁止操作**：
...
- 枚举字段必须列出所有枚举值及其中文含义
- 字段映射必须清晰，特殊转换逻辑必须说明
- 使用中文，确保文件使用 UTF-8 编码保存
- 文件应直接以标题开头，不要添加任何 HTML/SGML 注释
